...
public final class ByteBuffer implements Cloneable {
     ...
     * 是否是高位
     */
    private boolean bigByteOrder = false;

    public boolean isBigByteOrder() {
        return bigByteOrder;
    }

    public void setBigByteOrder(boolean bigByteOrder) {
        this.bigByteOrder = bigByteOrder;
    }
    ...
    /** 构造一个缓冲区,默认长度是1024 */
    public ByteBuffer(boolean bigByteOrder) {
        this(50);
        this.bigByteOrder = bigByteOrder;
    }
    /** 构造指定的长度的缓冲区 */
    ...
    /**
     * 写一个ByteBuffer的指定长度的内容 mask 是否websocke的掩码处理消息
     */
    public void writeByteBuffer(ByteBuffer buffer, int count, byte[] mask) {
        count = Math.min(count, buffer.available());
        ensureCapacity(length() + count);
        for (int i = 0; i < count; i++) {
            byte byts = buffer.data[buffer.readPos++];
            byts ^= mask[i % 4];
            data[writePos++] = byts;
        }
    }
    ...
    public String readUTFString() throws Exception {
        int len = readInt();
        //System.out.println("buffer readString: " + len);
        if (len == 0)
            return "";
    ...
    private void writeNumber(long value, int bytes) {
        if (!bigByteOrder) {
            ...
            for (int i = 0; i < bytes; i++) {
                numData[bytes - 1 - i] = (byte) (value >> i * 8);
            }
        }
     ...
     * @param len
     */
    public void skipReadPos(int len)
    {
        readPos+=len;
    }
    /**
